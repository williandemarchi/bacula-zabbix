...
2. Create the bash script file `/etc/bacula/bacula-zabbix.sh` by copying it from this repository and set the permissions as below:
  ```
  chown bacula:bacula /etc/bacula/bacula-zabbix.sh
  chmod 700 /etc/bacula/bacula-zabbix.bash
  ```
...
3. Edit the Bacula Director configuration file `/etc/bacula/bacula-dir.conf` to start the script at the finish of each job. To do this you need to change the lines described below in the Messages resource that is used by all the configured jobs:
  ...
  Messages {
    ...
    mailcommand = "/etc/bacula/bacula-zabbix.sh %i"
...
- **Bacula**:
  ...
  - http://www.bacula.org/9.0.x-manuals/en/main
...
- https://github.com/williandemarchi/bacula-zabbix
- williandemarchi@gmail.com
